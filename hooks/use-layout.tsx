...
const Layout = ({
  ...
  const [detectedForced, setDetectedForced] = React.useState<
    Layout | undefined
  >(undefined);
  ...
  React.useEffect(() => {
    if (isServer) return;
    if (forcedLayout) {
      setDetectedForced(forcedLayout);
      setLayoutState(forcedLayout);
      saveToLS(storageKey, forcedLayout);
      return;
    }

    const { hostname, pathname: locPathname } = window.location;
    const firstHostSegment = hostname.split(".")[0] || "";
    const isLabSubdomain = firstHostSegment.toLowerCase() === "lab";

    const currentPath = pathname ?? locPathname ?? "/";
    const isLabPath = currentPath === "/lab" || currentPath.startsWith("/lab/");

    if (isLabSubdomain || isLabPath) {
      setDetectedForced("full");
      setLayoutState("full");
      saveToLS(storageKey, "full");
    } else {
      setDetectedForced("fixed");
      setLayoutState("fixed");
      saveToLS(storageKey, "fixed");
    }
  }, [forcedLayout, storageKey, pathname]);
  ...
  const providerValue = React.useMemo(
    () => ({
      ...
      setLayout,
      forcedLayout: forcedLayout ?? detectedForced,
    }),
