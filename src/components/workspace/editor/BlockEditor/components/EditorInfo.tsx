...
export const EditorInfo = memo(
  // ({ characters, collabState, users, words, onSave }: EditorInfoProps) => {
  ({ characters, words, onSave }: EditorInfoProps) => {
    return (
      <div className="flex items-center gap-1">
                  ...
                  <div key={user.clientId} className="-ml-3">
                    <Tooltip title={user.name}>
                      <img
                        ...
                        src={user?.image ? user?.image : `https://api.dicebear.com/7.x/notionists-neutral/svg?seed=${
                          user.name
        ...
        <Button
          className="h-8 text-[12px] ml-2 gap-2 bg-yellow-500 hover:bg-yellow-600"
          onClick={() => onSave()}
